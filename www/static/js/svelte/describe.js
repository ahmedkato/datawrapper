(function (global, factory) {
	...
	function metaColumns(ref) {
	    ...
	    var column = ref.column;
	    var allColumns = ref.allColumns;

	    if (!allColumns) { return []; }
	    return allColumns.filter(function (col) { return col.name() !== column.name(); }).map(function (col) {
	        return {
	...
	function ComputedColumnEditor(options) {
		...
		this._recompute({ column: 1, columns: 1, allColumns: 1, metaColumns: 1 }, this._state);

		var self = this;
		var _oncreate = function() {
			var changed = { column: 1, columns: 1, allColumns: 1, metaColumns: 1, title: 1, name: 1 };
			oncreate.call(self);
	...
	assign(ComputedColumnEditor.prototype, proto);
	...
	ComputedColumnEditor.prototype._recompute = function _recompute(changed, state) {
		...
		if (changed.columns || changed.column || changed.allColumns) {
	...
	assign(CustomColumnFormat.prototype, proto);
	...
	function histogram() {
	  ...
	  histogram.thresholds = function(_) {
	    return arguments.length ? (threshold = typeof _ === "function" ? _ : Array.isArray(_) ? constant$2(slice$2.call(_)) : constant$2(_), histogram) : threshold;
	  };
	...
	var xScale_ = linear$1();
	var xScaleBand_ = band();
	var yScale_ = linear$1();
	...
	assign(Histogram.prototype, proto);
	...
	assign(Handsontable.prototype, proto);
	...
	function create_main_fragment$6(component, state) {
		...
		var if_block_2 = (state.searchResults.length > 0) && create_if_block_8(component);

		var if_block_3 = (state.search) && create_if_block_9(component, state);
		...
		return {
			...
			p: function update(changed, state) {
				...
				if (state.searchResults.length > 0) {
					if (!if_block_2) {
						if_block_2 = create_if_block_8(component);
						if_block_2.c();
				...
				if (state.search) {
					if (if_block_3) {
						...
						if_block_3 = create_if_block_9(component, state);
						if_block_3.c();
	...
	function create_if_block_2(component, state) {
		...
		var if_block = (state.columnFormat.isComputed) && create_if_block_3(component);
		...
		return {
			...
			p: function update(changed, state) {
				...
				if (state.columnFormat.isComputed) {
					if (!if_block) {
						if_block = create_if_block_3(component);
						if_block.c();
	...
	function create_if_block_7(component, state) {
		var h4, text_value = __("describe / locale-select / hed"), text, text_1, p, raw_value = __("describe / locale-select / body"), text_2, select, select_updating = false;
	...
	function create_if_block_6(component, state) {
		var h3, text_value = __("Make sure the data looks right"), text, text_1, p, raw_value = __("describe / data-looks-right"), text_2, checkbox_updating = {}, text_3, if_block_anchor;
		...
		var if_block = (state.showLocale) && create_if_block_7(component, state);
	...
	// (129:24) {#if searchResults.length > 0}
	...
	function create_if_block_9(component, state) {
		...
		function select_block_type_2(state) {
			if (state.searchResults.length > 0) { return create_if_block_10; }
			if (state.search) { return create_if_block_11; }
			return null;
		}
